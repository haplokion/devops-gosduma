...
def export_csv():
    with db.connect().cursor(named_tuple=True) as cursor:
        ...
        statistic=[]
        for i in statistics:
            string = {"id": i.statistic_id, "ФИО": get_fio(i.statistic_user), "Название поста": get_book_name(i.statistic_book), "Время посещения": i.statistic_created_at}
            statistic.append(string)

        print(statistic)
        data = load_data(statistic, ["id", "ФИО", "Название поста", "Время посещения"])
    ...
    if request.method == "POST":
        ...
        try:
            if not current_user.review(book_id):
                raise Exception("Отзыв существует")
